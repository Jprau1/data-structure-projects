...
Test printTests();
Test basicListTests();
Test listAssignmentTests();
Test listReverseTests();
Test iteratorTests();
Test stockSalesTests();
...
int main()
{
	...
	cout << endl << tests.getPassFailLogsAsString() << endl;
	...
	t.assertNoException(
		[&t]()
		{
			//
			{
				...
				//	Get an exception if we try to oversell this book
				t.assertException(
					[&t, &store]()
					{
						//store.printInventory();
						store.sellToCustomer("1435769244", 2000, 6);
					}, 1, "Should find exception if we oversell"
				);
